...
async function handleGetPatient() {
  const response = await getPatient(token, 5, "")
  const json = await response.json()
...
async function handleDeletePatient(mr: string) {
  const response = await deletePatient(token, mr)
  const json = await response.json()
...
<template>
  <section class="anim-slide" ref="edits">
      ...
      <form class="form-data-custom" v-on:submit.prevent="handleGetSearchPatient">
        ...
        <div class="center" style="justify-content: flex-start; align-items: flex-end; padding-left: 1rem;">
          <div style="padding: 0.5rem;">
            ...
            <input type="text" id="parent_name" placeholder="No RM/Nama Pasien" v-model="search">
          </div>
          <div style="padding: 0.5rem;">
            <div style="margin-bottom: 0.5rem;">
              <label for="limit1">Limit</label>
            </div>
            <input type="number" id="limit1" placeholder="limit data" v-model="limit">
          </div>
          <button>Cari</button>
        </div>
      </form>
    ...
    <div style="width: 100%; max-height: 20rem; overflow-x: scroll; overflow-y: scroll; scrollbar-width: thin;">
      <table class="table-custom">
        ...
        <tbody>
          <tr v-for="patient in patientDatas" :key="patient.medical_record">
            <td>
              <button class="button-action" @click="edit(patient), mr = patient.medical_record, editAction = true">Edit</button>
              <button class="button-action" @click="handleDeletePatient(patient.medical_record)">Delete</button>
            </td>
