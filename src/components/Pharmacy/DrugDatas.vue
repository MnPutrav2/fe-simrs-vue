...
async function handleGetDrugData() {
  const response = await getDrugData(token, search.value, limit.value)
  const json = await response.json()
...
function handleEditDrugData(data: ResponseDrugData) {
  ...
  const {distributor_id, ...filter} = data
  Object.assign(drug, filter);
...
async function handleUpdateDrugData(id: string) {
  ...
  const response = await updateDrugData(token, dt.value)
  const json = await response.json()
...
async function handleDeleteDrugData(id: string) {
  const response = await deleteDrugData(token, id)
  const json = await response.json()
      ...
      <form class="form-data-custom" v-on:submit.prevent="handleGetDrugData">
        ...
        <div class="center" style="justify-content: flex-start; align-items: flex-end; padding-left: 1rem;">
          <div style="padding: 0.5rem;">
            ...
            <input type="text" id="sc" v-model="search" placeholder="Nama obat">
          </div>
          <div style="padding: 0.5rem;">
            <div style="margin-bottom: 0.5rem;">
              <label for="lm">Limit</label>
            </div>
            <input type="number" id="lm" v-model="limit" placeholder="limit">
          </div>
      ...
      <table class="table-custom">
        ...
        <tbody>
          <tr v-for="(drug, index) in drugDatas" :key="drug.id">
            <td>{{ index + 1 }}</td>
            <td>
              <button class="button-action" @click="handleEditDrugData(drug); bool = true">Edit</button>
              <button class="button-action" @click="handleDeleteDrugData(drug.id)">Delete</button>
            </td>
            <td>{{ drug.id }}</td>
            <td>{{ drug.name }}</td>
